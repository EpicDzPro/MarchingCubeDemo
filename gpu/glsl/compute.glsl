...
layout(set = 0, binding = 0, std430) restrict writeonly buffer TriangleBuffer
{
	float data[];
}
triangleBuffer;

layout(set = 0, binding = 1, std430) restrict readonly buffer VoxelBuffer
{
	float data[];
}
voxelBuffer;
...
vec4 evaluate(vec3 point)
{   

    int range = params.resolution + 1;
    float id = point.x *  range * range  + point.y * range + point.z;
	float density = voxelBuffer.data[int(id)];
	vec3 local_point = (point - params.resolution / 2) * params.size / params.resolution;
    
	return vec4(local_point, density);
}
...
void main()
{
	...
	for (int i = 0; i < numIndices; i += 3) {
		...
		int v0 = lutter[offset + i];
		int v1 = lutter[offset + 1 + i];
		int v2 = lutter[offset + 2 + i];
		...
		uint index = atomicAdd(params.counter, 1u);
		triangleBuffer.data[index*12] = currTri.a.x;
        triangleBuffer.data[index*12+1] = currTri.a.y;
        triangleBuffer.data[index*12+2] = currTri.a.z;

        triangleBuffer.data[index*12+3] = currTri.b.x;
        triangleBuffer.data[index*12+4] = currTri.b.y;
        triangleBuffer.data[index*12+5] = currTri.b.z;

        triangleBuffer.data[index*12+6] = currTri.c.x;
        triangleBuffer.data[index*12+7] = currTri.c.y;
        triangleBuffer.data[index*12+8] = currTri.c.z;

        triangleBuffer.data[index*12+9] = normal.x;
        triangleBuffer.data[index*12+10] = normal.y;
        triangleBuffer.data[index*12+11] = normal.z;

        //normalBuffer.data[index] = -normalize(cross(ab,ac));
